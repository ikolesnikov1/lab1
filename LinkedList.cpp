...
LinkedList::LinkedList(const LinkedList &extra) {
    tail = new Node();
    tail -> prev = tail;
    tail -> next = tail;
    for (LinkedList::const_iterator it = extra.cbegin(); it != extra.cend(); it++) {
        push_back(*it);
    }
}

LinkedList::LinkedList(LinkedList &&extra) noexcept {
    tail = extra.tail;
    count = extra.count;
    extra.tail = nullptr;
    extra.count = 0;
...
LinkedList &LinkedList::operator=(const LinkedList &extra) {
    clear();
    for (LinkedList::const_iterator it = extra.cbegin(); it != extra.cend(); it++) {
        push_back(*it);
    }
...
LinkedList &LinkedList::operator=(LinkedList &&extra) {
    clear();
    delete tail;
